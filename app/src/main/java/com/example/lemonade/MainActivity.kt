...
class MainActivity : AppCompatActivity() {
     ...
     */
    private fun clickLemonImage(inState: String) {
        ...
        var lemonState = inState
        when(inState) {
            SELECT -> {
                lemonState = "squeeze"; lemonSize = lemonTree.pick()}
            SQUEEZE -> {
                if (lemonSize == 0) {
                    lemonState = "drink"
                    lemonSize = -1
